...
namespace MvcRouteTester.Common
{
    internal class Verifier
    {
        ...
        private void VerifyValue(RouteValue expectedValue, RouteValue actualValue)
        {
            ...
            else if (expectedValue.Value is Enum)
            {
                VerifyEnumValue(expectedValue, actualValue);
            }
            else
            {
        ...
        private void VerifyEnumValue(RouteValue expectedValue, RouteValue actualValue)
        {
            int? expectedInt = Convert.ToInt32(expectedValue.Value);
            int? actualInt = null;

            try
            {
                actualInt = Convert.ToInt32(actualValue.Value);
            }
            catch { }
            if (actualInt == null)
            {
                var parseFailErrorMessage = string.Format(
                    "Actual value '{0}' could not be parsed as enum at url '{1}'.",
                    actualValue, url);
                Asserts.Fail(parseFailErrorMessage);
            }
            else
            {
                if (expectedInt.Value != actualInt.Value)
                {
                    var mismatchErrorMessage = string.Format("Expected '{0}', not '{1}' for '{2}' at url '{3}'.",
                        expectedValue.ValueAsString, actualValue.ValueAsString, expectedValue.Name, url);
                    Asserts.Fail(mismatchErrorMessage);
                }
            }
            expectationsDone++;
